...
impl Data {
    ...
    pub fn register(&mut self, name: &str, pattern: &str, action: MessageReply) {
        self.search_patterns.insert(name.to_string(), Regex::new(pattern).unwrap());
        self.last_responses.insert(name.to_string(), Mutex::new(DateTime::<Utc>::from_timestamp(0, 0).unwrap()));
        self.response_functions.insert(name.to_string(), action);
    }

    fn get_response_types(&self) -> Vec<String> {
        self.search_patterns.keys().map(|x| x.clone()).collect::<Vec<String>>()
    }
    ...
    pub fn check_should_respond(&self, message: &Message) -> Option<String> {
        for name in self.get_response_types() {
            if self.search_patterns.get(&name).unwrap().is_match(&message.content) {
                return Some(name);
            }
        }
        return None;
    ...
    pub async fn run_action(&self, name: &str, message: &Message, ctx: &serenity::Context) -> Result<(),Error> {
        let action = self.response_functions.get(name).unwrap();
        match action(message,ctx) {
            Text(text) => message.reply(ctx, text).await?,
            Image(path) => message.channel_id.send_message(ctx, |m| {
            ...
            TextPlusImage(text, path) => message.channel_id.send_message(ctx, |m| {
                m.reference_message(message);
                m.allowed_mentions(|am| {
                    am.replied_user(false);
                    am
