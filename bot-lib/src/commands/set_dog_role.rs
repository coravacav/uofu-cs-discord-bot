...
#[poise::command(slash_command, rename = "woof", ephemeral = true, guild_only)]
pub async fn add_dog_role(ctx: PoiseContext<'_>) -> Result<()> {
    let author = ctx.author();
    let guild = ctx.guild().ok_or_eyre("Couldn't get guild")?.id;
    let role_id = RoleId::from(ctx.data().config.read().await.ids.dog_react_role_id);
...
#[poise::command(slash_command, rename = "muzzle", ephemeral = true, guild_only)]
